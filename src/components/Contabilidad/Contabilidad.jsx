...
function Contabilidad({ onVolver }) {
  ...
  const AREA_ID_CONTABILIDAD = 8; 

  useEffect(() => {
    const cargarPlantillas = async () => {
      try {
        const res = await axios.get("/api/templates", {
          params: { area_id: AREA_ID_CONTABILIDAD }
        });

        console.log("Plantillas Contabilidad:", res.data);
        setPlantillas(res.data);
      } catch (error) {
        console.error("Error cargando plantillas de contabilidad", error);
      } finally {
        setLoading(false);
      }
    };
...
console.log("Contenido render:",plantillaSeleccionada?.contenido);
  ...
  return (
    <div className="contabilidad-container">
      <h1 className="main-title">Contabilidad</h1>
