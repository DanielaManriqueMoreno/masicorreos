...
function SistemaCitas({ onVolver }) {
  ...
  const AREA_ID_CITAS = 5; 

  useEffect(() => {
    const cargarPlantillas = async () => {
      try {
        const res = await axios.get("/api/templates", {
          params: { area_id: AREA_ID_CITAS }
        });

        console.log("Plantillas Citas:", res.data);
        setPlantillas(res.data);
      } catch (error) {
        console.error("Error cargando plantillas de citas", error);
      } finally {
        setLoading(false);
      }
    };
